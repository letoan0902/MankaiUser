...
let listQuestion = user.course.find(course => course.id ==12).lessons.find(lesson => lesson.id == 103).detail;
...
function loadQuestion() {
    ...
    <div class="quiz-header">
        ...
        <div class="quiz-progress">Câu ${currentQuestion + 1}/${totalQuestions}</div>
    </div>
    <div class="quiz-content">
        <div class="audio-section">
            <div class="audio-title">${question.name}</div>
            <div class="audio-button">
                ${question.audio ? `<img class="audio-icon" src="/team2-mankai-user/assets/icons/volume-exam.png" alt="Audio icon">` : `<p class="text-audio">${question.text}</p>`}
            </div>
        </div>
    ...
    options.forEach(option => {
        option.addEventListener('click', () => {
            ...
            options.forEach(opt => {
                ...
                opt.querySelector('.answer-badge').className = 'answer-badge badge-neutral';
...
function checkAnswer() {
    ...
    if (isCorrect) {
        ...
        selectedOption.querySelector('.answer-badge').className = 'answer-badge badge-correct';
        selectedOption.querySelector('.answer-text').className = 'answer-text text-correct';
