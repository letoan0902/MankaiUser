...
console.log(scoreExamVocab,scoreExamGrammar,scoreExamListen);
...
function renderExamGrammar(){
    let html = user.studyMankai[2].detail[0].exams[0].structure[2].questions[0].list.map((data, index) => {
        return `
...
function checkAnswer(){
    isChecked = true;
    let questions = user.studyMankai[2].detail[0].exams[0].structure[2].questions[0].list;
    let containerListAnswer = containerAllAnswer.querySelectorAll(".list-answer");
    containerListAnswer.forEach((container, indexQuestion) => {
        let answers = container.querySelectorAll(".answer-item");
    ...
    localStorage.setItem("scoreExamListen", JSON.stringify(scoreExamListen));
