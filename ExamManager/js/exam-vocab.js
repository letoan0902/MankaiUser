...
function attachAnswerEvents() {
  let questionContainers =
    containerListAnswer.querySelectorAll(".container-answer");
...
let timeOutVocab =
  user.studyMankai[2].detail[indexExamN].exams[indexExamS].structure[0].time;
let timeOutGrammar =
  user.studyMankai[2].detail[indexExamN].exams[indexExamS].structure[1].time;
let timeOutListen =
  user.studyMankai[2].detail[indexExamN].exams[indexExamS].structure[2].time;
...
function checkAnswer() {
  ...
  questionContainers.forEach((container, questionsIndex) => {
    ...
    let correcAnswerIndex = questions[questionsIndex].select.findIndex(
      (option) => option.check === true
    );
    answers.forEach((answer, answerIndex) => {
    ...
    if (
      userSelectedAnswerIndex !== undefined &&
      userSelectedAnswerIndex === correcAnswerIndex
    ) {
      scoreExamVocab++;
    }
  });
