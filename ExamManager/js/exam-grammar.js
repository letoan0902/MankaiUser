...
btnSubmitExam.addEventListener("click", function(){
    ...
    let totalquestionListen = user.studyMankai[2].detail[indexExamN].exams[indexExamS].structure[1].questions.length * user.studyMankai[2].detail[indexExamN].exams[indexExamS].structure[1].questions[0].list.length;

    resultVocab.innerHTML = `${scoreExamVocab}/${totalquestionVocab}` ;
    resultGrammar.innerHTML = `${scoreExamGrammar}/${totalquestionGrammar}`;
    resultListen.innerHTML = `${scoreExamListen}/${totalquestionListen}` ;
});
...
function renderExamGrammar() {
    ...
    pictureListenings.forEach((pictureListening) => {
        let questionDiv = pictureListening.closest(".question");
        let rectangle = questionDiv.querySelector(".rectangle");
        let timeListenVolume = questionDiv.querySelector(".timeListen-volume");
...
function checkAnswer() {
    ...
    localStorage.setItem("scoreExamGrammar", scoreExamGrammar.toString());
}
